...
docker cp src/spark_app.py spark-master:/opt/spark-apps/ >> "$LOG_FILE" 2>&1
log_success "spark_app.py copiado"

# Copiar spark_supabase.py
if [ -f "src/spark_supabase.py" ]; then
    docker cp src/spark_supabase.py spark-master:/opt/spark-apps/ >> "$LOG_FILE" 2>&1
    log_success "spark_supabase.py copiado"
fi
